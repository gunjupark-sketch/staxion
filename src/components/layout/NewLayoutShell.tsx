        ...
        <div className="fixed inset-0 z-[60] flex flex-col bg-white lg:hidden">
          ...
          <div className="mx-5 mt-5 mb-4 flex items-center gap-3 rounded-xl p-4" style={{ background: C.surface }}>
            ...
            <div className="flex-1">
              <p className="text-base font-bold" style={{ color: C.dark }}>회원</p>
              <p className="text-[12px]" style={{ color: C.textMuted }}>로그인 후 이용해주세요</p>
            </div>
            <Link href="/mypage" onClick={() => setSidebarOpen(false)}
              className="rounded-lg px-3 py-1.5 text-xs font-semibold" style={{ background: C.neon, color: C.dark }}>
              마이페이지
            </Link>
          </div>
          ...
          <div className="px-5 py-4 border-t" style={{ borderColor: C.border }}>
            <Link href="/login" onClick={() => setSidebarOpen(false)}
              className="block mb-3 text-sm" style={{ color: C.textMuted }}>
              로그인 / 로그아웃
            </Link>
