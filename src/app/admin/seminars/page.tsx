...
export default function SeminarsPage() {
  ...
  const fetchSeminars = useCallback(async () => {
    ...
    if (error) console.error("세미나 로드 실패:", error);
  ...
  const bulkDelete = async () => {
    if (!confirm(`선택한 ${selectedIds.size}개 세미나를 삭제하시겠습니까?`)) return;
    ...
    else { setSelectedIds(new Set()); fetchSeminars(); }
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">전체 {seminars.length}개</p>
        </div>
        <Button
          onClick={() => router.push("/admin/seminars/new")}
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((seminar) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`/admin/seminars/${seminar.id}`)}
                  >
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <input
                        type="checkbox"
                        checked={selectedIds.has(seminar.id)}
                        onChange={() => toggleOne(seminar.id)}
                        className="h-4 w-4 rounded border-gray-300 accent-brand-neon-safe"
                      />
                    </TableCell>
                    <TableCell>
                      <Badge variant="secondary" className={seminar.is_published ? "bg-green-50 text-green-600" : "bg-gray-100 text-gray-500"}>
                        {seminar.is_published ? "공개" : "비공개"}
                      </Badge>
                    </TableCell>
                    <TableCell className="font-medium max-w-xs truncate">{seminar.title}</TableCell>
                    <TableCell className="text-text-muted whitespace-nowrap">
                      {seminar.date ? new Date(seminar.date).toLocaleDateString("ko-KR") : "-"}
                    </TableCell>
                    <TableCell className="text-text-muted">{seminar.location || "-"}</TableCell>
